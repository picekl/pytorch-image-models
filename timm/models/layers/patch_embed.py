...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
        img_size = (in_chans, img_size[0], img_size[1])
        patch_size = (in_chans, patch_size, patch_size)
        self.img_size = img_size
        self.patch_size = patch_size
    ...
    def forward(self, x):
        B, C, D, H, W = x.shape
        _assert(D == self.img_size[0], f"Input image depth ({D}) doesn't match model ({self.img_size[0]}).")
